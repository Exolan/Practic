function loadCart(modalBody) {
  ...
  let arrayCurt = JSON.parse(localStorage.getItem("curt"));
  if (arrayCurt.length != 0) {
    for (let index = 0; index < arrayCurt.length; index++) {
      const foodRow = document.createElement("div");
      foodRow.className = "food-row";
      foodRow.innerHTML = `<span class="food-name">${arrayCurt[index].name}</span>
            <strong class="food-price">${arrayCurt[index].price} ₽</strong>
